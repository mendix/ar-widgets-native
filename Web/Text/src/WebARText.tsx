...
export function WebARText(props: WebARTextContainerProps): React.ReactElement {
    ...
    const handleSceneLoaded = async (scene: Scene) => {
        ...
        loadedFontData.current = fontData;
        const newMesh = MeshBuilder.CreateText("myText", props.mxText.value ?? "Hello World !! @ #$ % é", fontData, {
            resolution: 64,
            depth: 10
        });
        if (newMesh) {
